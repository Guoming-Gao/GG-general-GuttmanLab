...
from step05_link_trajectories import LEGACY_COLUMNS, assign_track_labels, legacy_table, link_detections
...
def test_laptrack_cutoff_minimum_length_and_legacy_contract():
    ...
    tracks, summary, edges = link_detections(pd.DataFrame(rows), cfg)
    assert len(summary) == 1
    assert summary.loc[0, "max_step_px"] <= 5
    assert set(edges["edge_type"]) == {"adjacent"}
...
@pytest.mark.skipif(importlib.util.find_spec("laptrack") is None, reason="laptrack not installed")
def test_laptrack_closes_one_missing_frame_within_two_frame_limit():
    rows = []
    for spot_id, frame in enumerate([0, 1, 2, 4, 5, 6]):
        rows.append({
            "dataset": "d", "condition": "c", "fov": "f", "channel": "spt",
            "spotID": spot_id, "frame": frame, "t": frame, "x": float(frame), "y": 0.0,
            "spotiflow_probability": 0.8, "R": 2.5, "meanIntensity": 10,
            "medianIntensity": 9, "minIntensity": 2, "maxIntensity": 15,
            "totalIntensity": 100, "stdIntensity": 3, "contrast": 0.5, "SNR": 3,
            "detection_cell_id": 1,
        })
    cfg = {"dataset": "d", "laptrack": {"metric": "sqeuclidean", "max_link_distance_px": 5,
           "gap_closing": True, "gap_closing_max_frame_count": 2, "min_track_length": 5}}
    tracks, summary, edges = link_detections(pd.DataFrame(rows), cfg)
    assert len(summary) == 1
    assert (edges["edge_type"] == "gap_closing").sum() == 1
